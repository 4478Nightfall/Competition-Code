...
// Spins the intake in the specified direction (1 for forward, -1 for reverse)
...
// Stops the intake and holds its position
void stopIntake()
{
    intake.set_brake_mode(MOTOR_BRAKE_HOLD);
    intake.brake();
...
// Toggles the back gate pneumatic actuator
void handleBackGate()
{
    backGate.set_value(backGate.get_value() == LOW ? HIGH : LOW);
}

// Toggles the front gate pneumatic actuator
void handleFrontGate()
{
    frontGate.set_value(frontGate.get_value() == LOW ? HIGH : LOW);
}

// Toggles the match load mechanism pneumatic actuator
void handleMLMech()
{
    matchloadMech.set_value(matchloadMech.get_value() == LOW ? HIGH : LOW);
}

// Slew rate limiter to prevent sudden changes in motor output
// val: target value, fwdVal: direction of movement
// Returns a value that changes gradually toward val
// Used to smooth acceleration/deceleration
// slewStep controls how quickly the value can change
...
// PID drive function for straight movement
// fwdVal: distance to move (inches), limit: max output, timeout: ms before giving up
void drivePID(double fwdVal, double limit, double timeout)
{
    ...
    // Convert forward value to target degrees for motor encoders
    double num = fwdVal;
...
// PID turn function for in-place turning
// turnVal: target heading (degrees)
